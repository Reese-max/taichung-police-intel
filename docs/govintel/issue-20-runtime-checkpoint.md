...
- Restore all six files from one pinned commit or fail before writing any file. Symlinks and oversized blobs are rejected. The local restore receipt is written last.
...
- After deployment, all five public data files must match the exact generation before acknowledging PUBLISHED. The sixth file is internal durable V2 state, not an HTTP resource. Probe uses the README's fixed Pages origin/path, rejects redirects and arbitrary URLs, and has bounded time/byte/retry limits.
...
- Later source/schema changes must version this fixed six-file contract; no candidate source is promoted by this patch.
