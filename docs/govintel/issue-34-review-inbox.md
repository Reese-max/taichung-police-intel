...
- `scripts/system-health.py` 會把目前 `source-status.json` 的 `STALE_SOURCE / PARTIAL_SOURCE` 與 schema-drift review candidates 一起投影；同一 fingerprint 的 reason 變更會留下 `RECLASSIFIED` audit，而不是沿用舊分類。

## Verification

`npm run check`：`VERIFY_OK mode=full required=112 specs=4 secrets=0`；覆蓋 dedupe、reason reclassification、source-health projection、local decision、版本變更重開、storage round-trip、binding tamper rejection 與 Dashboard boundary copy。
