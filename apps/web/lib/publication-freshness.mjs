...
export function assessPublication(publication, sourceStatus, nowMs, maxAgeMs = MAX_SNAPSHOT_AGE_MS) {
  ...
  if (!Array.isArray(sources) || sources.length === 0 ||
      sources.some(s => !s || typeof s !== "object" || typeof s.source_id !== "string" || !s.source_id) ||
      new Set(sources.map(s => s.source_id)).size !== sources.length) return result("UNKNOWN", "缺少來源覆蓋資料。");
