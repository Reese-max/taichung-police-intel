...
test('recent snapshot is not automatically verified publication',()=>{const s=m.inspectSnapshot(...fixture(),now);assert.equal(s.status,'SNAPSHOT_RECENT');assert.equal(s.publication_verified,false);});
...
test('UI route is isolated with same-origin assets and explicit boundaries',async()=>{const html=await readFile(new URL('../public/workspace-preview/index.html',import.meta.url),'utf8');assert.match(html,/lang="zh-Hant"/);assert.match(html,/connect-src 'self'/);assert.match(html,/\.\/workspace\.mjs/);assert.doesNotMatch(html,/https:\/\/(?:fonts|cdn)/);});
