...
test('UI route is isolated with same-origin assets and explicit boundaries',async()=>{const html=await readFile(new URL('../public/workspace-preview/index.html',import.meta.url),'utf8');assert.match(html,/lang="zh-Hant"/);assert.match(html,/connect-src 'self'/);assert.match(html,/\.\/dialog-trigger-focus\.mjs/);assert.match(html,/\.\/workspace\.mjs/);assert.match(html,/\.\/skip-focus\.mjs/);assert.doesNotMatch(html,/https:\/\/(?:fonts|cdn)/);});
