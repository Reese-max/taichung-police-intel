...
def run(args: list[str], cwd: Path, timeout: int = 600, env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    return subprocess.run(args, cwd=cwd, capture_output=True, text=True, encoding="utf-8", timeout=timeout, env=env)
...
def make_handler(ctx: dict[str, Any], serve_dir: Path):
    ...
    class Handler(SimpleHTTPRequestHandler):
        ...
        def _gateway_post(self, path: str) -> None:
            if self._query_down():
                return _json_response(self, {"status": "QUERY_UNAVAILABLE", "error": "query service unavailable"}, 503)
            try:
                length = int(self.headers.get("Content-Length", "-1"))
                if length < 0 or length > gateway_module.MAX_REQUEST_BYTES:
                    raise ValueError("request body exceeds bounded limit")
                payload = json.loads(self.rfile.read(length).decode("utf-8"))
                if not isinstance(payload, dict):
                    raise ValueError("request body must be an object")
                gateway = served_gateway()
                if path == "/query":
                    response = gateway.execute(payload.get("tool"), payload.get("arguments"))
                else:
                    response = gateway_module.dispatch_mcp(gateway, payload)
                return _json_response(self, response)
            except gateway_module.GatewayError as error:
                return _json_response(self, {"schema_version": 1, "error": {"code": error.code, "message": error.message}}, error.status)
            except (UnicodeDecodeError, json.JSONDecodeError, TypeError, ValueError) as error:
                return _json_response(self, {"schema_version": 1, "error": {"code": "INVALID_ARGUMENTS", "message": str(error)}}, 400)
        ...
        def do_POST(self):
            ...
            if path in ("/query", "/mcp"):
                return self._gateway_post(path)
            self.send_error(404)
...
def run_sabotage_check(root: Path, evidence_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        marker = 'SOURCE_POLICY = ROOT / "scripts/source-policy.py"'
        if marker not in text:
            return check_record("sabotage_detection", False, "SOURCE_POLICY anchor not found in query-store.py copy")
        victim.write_text(text.replace(marker, 'SOURCE_POLICY = ROOT / "scripts/source-policy-sabotaged.py"', 1), encoding="utf-8")
