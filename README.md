...
Review Inbox uses `intel_v2/review.py` and `scripts/review-inbox.py` for deterministic fingerprints, deduplication, claim/decision audit, source-version reopening, and bounded homepage projection. Local audit state keeps reviewer/assignment details, while the public health receipt exposes only safe IDs and hashes. It accepts schema-drift candidates without auto-resolving missing or failed sources; the first version is local-first and does not add RBAC or notifications.
