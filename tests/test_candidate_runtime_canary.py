...
def collector(fail=None):
    def collect(session, source_id, start, end, existing, *, max_details):
        ...
        return {"manifest_sha256": "a" * 64, "source_health": "PASS", "window_completeness": "PARTIAL",
                "window_item_count": 2, "snapshot_item_count": 3,
                "snapshots": [{"purpose": "LIST", "body": b"not-public"}, {"purpose": "DETAIL"}]}
    return SimpleNamespace(NEWS_LIST_SOURCES={s: {"list_url": "https://official.example.test/"} for s in module.SOURCES},
...
class CanaryContractTests(unittest.TestCase):
    def test_success_is_candidate_observation_not_promotion(self):
        report = module.run_canary(collector(), list(module.SOURCES), NOW, session_factory=FakeSession)
    ...
    def test_one_source_failure_preserves_other_observations_and_null_counts(self):
        report = module.run_canary(collector("S-019"), list(module.SOURCES), NOW, session_factory=FakeSession)
